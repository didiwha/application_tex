					                    	...
					                    	'<select name="scaner" class="form-control" required>' +
					                    		<?php if(!empty(get_session_user_scaners_array($this))): ?>
						                    		<?php foreach (get_session_user_scaners_array($this) as $scaner): ?>
						                    			<?php if($scaner->scaner_id === get_session_default_scaner_array($this)[0]->id): ?>				
						                    					'<option value="<?=$scaner->scaner_id;?>" selected><?=$scaner->etablissement_short;?> - <?=$scaner->service_short;?> - <?=$scaner->libelle;?></option>' +
						                    			<?php else: ?>
